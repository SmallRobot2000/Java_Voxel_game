...
import static com.badlogic.gdx.graphics.TextureData.TextureDataType.*;
...
public class EnvironmentCubemap implements Disposable{
    ...
    public EnvironmentCubemap (Pixmap cubemap) {
        ...
        for(int i=0; i<6; i++) data[i] = new Pixmap(w/4, h/3, com.badlogic.gdx.graphics.Pixmap.Format.RGB888);
        for(int x=0; x<w; x++)
            for(int y=0; y<h; y++){
                //-X
